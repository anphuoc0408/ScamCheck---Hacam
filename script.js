// Khai báo ảnh QR cố định ở đầu file script.js
const qrImage = new Image();
qrImage.src = "z7975734935203_e5e2a05076b2fa6475e2396110405475.jpg";
...
async function renderWarningCard(message, result) {
  ...
  function drawContent(dryRun = false) {
    ...
    if (!dryRun) {
      ...
      // Font hệ thống cao cấp hiển thị tiếng Việt mượt mạc
      ...
      // ========================================================
      // ĐOẠN THÊM MỚI: Vẽ ảnh QR thật của bạn vào góc dưới cùng bên phải
      // Kích thước Canvas của bạn là 1080 x 1350 (dựa vào file index.html)
      // ========================================================
      const qrSize = 180; // Chiều rộng và cao của mã QR (180x180px)
      const qrX = width - qrSize - 130; // Cách lề phải 130px để căn đều với khung trắng
      const qrY = warningCardCanvas.height - qrSize - 450; // Cách lề dưới 130px

      // Kiểm tra nếu ảnh đã tải xong thì vẽ lên canvas luôn
      if (qrImage.complete) {
        ctx.drawImage(qrImage, qrX, qrY, qrSize, qrSize);
      } else {
        // Dự phòng trường hợp mạng chậm ảnh chưa tải xong
...
function setView(viewName) {
  ...
  if (
    isSameView ||
    !previousView ||
    previousView.classList.contains("hidden")
  ) {
  ...
  previousView.classList.add(
    movingForward ? "view-exit-left" : "view-exit-right",
  );
  activeViewName = nextViewName;
  ...
  viewSwitchTimer = window.setTimeout(() => {
    ...
    nextView.classList.add(
      movingForward ? "view-enter-right" : "view-enter-left",
    );
...
btnResponds.forEach((button) => {
  listen(button, "click", async () => {
    ...
    responderGuidance.innerHTML =
      "<div class='spinner' style='width: 20px; height: 20px; display: inline-block; vertical-align: middle; margin-right: 8px;'></div> Đang lấy hướng dẫn khẩn cấp...";
    ...
    try {
      ...
      responderGuidance.innerHTML =
        "⚠️ Lỗi kết nối hệ thống khi lấy hướng dẫn khẩn cấp. Bác hãy gọi ngay cho Ngân hàng để khóa tài khoản hoặc ra đồn Công an gần nhất để được hỗ trợ.";
      responderGuidance.className = "responder-guidance guidance-otp";
    }
  });
});
